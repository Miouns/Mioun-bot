...
// const db = requirerequire('quick.db')
...
module.exports = async (client, message) => {
  ...
  let maintance = db.get('maintance');
  ...
  let defPref = client.config.prefix;
  let cusPref = db.get(`prefix.${message.guild.id}`);
  if(!cusPref) {
    var prefix = defPref;
  } else {
    var prefix = cusPref;
  ...
  */
  ...
  if(message.content.startsWith(prefix.length) && maintance) return message.channel.send("MAINTANCE TOD")
  ...
  try {
    ...
    console.log(`${sender.tag} (${sender.id} | ${message.guild.id}) ran a command: ${cmd}`);
  }
